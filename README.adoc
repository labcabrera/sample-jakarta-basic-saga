  ...
  cloud:
    stream:
      ...
      function:
        definition: foo;bar
      bindings:
        processPersonModificationResult-in-0:
          destination: foo.exchange
          consumer:
            concurrency: 10
            max-attempts: 1
          group: person-modification-result
        sendPersonModificationNotification-out-0:
          destination: bar.exchange
      rabbit:
        bindings:
          processPersonModificationResult-in-0:
            consumer:
              bindingRoutingKey: person-modification
          sendPersonModificationNotification-out-0:
            producer:
              routingKeyExpression: keyB
          processPersonModificationResult-out-0:
            producer:
              routingKeyExpression: keyA
----
